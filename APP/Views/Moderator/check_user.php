<?php
	if (!isset($_SESSION['login']) || $_SESSION['login']->acces != 10) 
    {
		...
		if (isset($_POST) && !empty($_POST)) 
		{
			if (isset($_POST['ban'])) 
			{
				$ban = new APP\Moderator\UnBan($_POST['title'],$_POST['chapo'],$_POST['content'],intval($_SESSION['login']->ID));
				$message = $post->insert();
			}
			elseif (isset($_POST['unban'])) 
			{
				$unban = new APP\Moderator\UnBan($_POST['title'],$_POST['chapo'],$_POST['content'],intval($_SESSION['login']->ID));
				$message = $post->insert();
			}
		}
?>
